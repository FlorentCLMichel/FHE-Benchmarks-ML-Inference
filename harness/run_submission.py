...
def main():
    ...
    # Skipping this step as there is no database encryption in this benchmark
    ...
    for run in range(num_runs):
        ...
        # 6. Client-side: Generate a new random input using harness/generate_input.py
        ...
        subprocess.run(cmd, check=True)
        utils.log_step(6, "Input generation")

        # 7. Client-side: Preprocess input using exec_dir/client_preprocess_input
        subprocess.run([exec_dir/"client_preprocess_input", str(size)], check=True)
        utils.log_step(7, "Input preprocessing")

        # 8. Client-side: Encrypt the input
        subprocess.run([exec_dir/"client_encode_encrypt_input", str(size)], check=True)
        utils.log_step(8, "Input encryption")
